...
`--date` 同时是报告日期和行情截止日期，晚于该日期的行情不会参与检测。如果现有状态或输入行情晚于该截止日，本次运行自动进入“历史回放只读模式”：生成报告，但不修改实时告警状态，也不发送通知。同一日期重复运行时，新的运行结果会追加到当日报告并带运行时间；原始报警和后续静默、降级结果都会保留。推送正文只包含本次运行的新报警。
