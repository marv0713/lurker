# A-Share Macro Flow Radar — 周报与月报设计

Date: 2026-06-04

## Goal

在日报（`professional_flow_daily`）之外，增加两个更高维度的宏观资金视角：

- **周报**（`weekly_macro_flow`）：每周五收盘后运行，回答"这一周资金是不是在持续进场"。
- **月报**（`monthly_macro_flow`）：每月最后一个交易日运行，回答"当前是否处于牛市进度条上升阶段"。

日报侧重个股和板块的当日动向；周报和月报侧重宏观流动性趋势，用来判断日报信号的大背景。

---

## 周报：`weekly_macro_flow`

### 报告标题

`宏观资金周报`

### 回答的问题

- 这一周资金是不是持续在进场？
- 哪些板块连续跑赢？哪些只是反弹？
- 超大单周度净流入是否比价格表现更强？
- ETF 是净申购还是净赎回？

### 数据源

#### ETF 净申购趋势

- 核心 ETF（510300、510500、159915 以及可配置的 A500 ETF）的周度申购/赎回变化。
- **份额主数据源**：Tushare `fund_share`。字段 `fd_share` 的单位为万份；
  使用 `trade_date`、`start_date`、`end_date` 拉取真实历史份额。
- AkShare `fund_etf_hist_em` **只提供成交量/成交额，不提供 ETF 份额**，
  不得再用成交量变化冒充申购赎回。
- 成交额仅作为确认项：真实份额增加但成交额未放大时，仍记为净申购，
  但标注“成交确认偏弱”。
- 成交确认按最近 5 个完整交易日的 ETF 成交额合计与前 5 个交易日合计比较：
  本周合计大于前周为 `expanded`，否则为 `not_expanded`；任一窗口不足 5 日
  为 `unknown`。确认项不改变真实份额方向。
- “份额平稳但价格上涨”的价格条件使用四只核心 ETF 周度收盘价：至少
  3/4 只本周最后交易日相对前周最后交易日上涨超过 0.5%，定义为价格上涨；
  任一 ETF 价格端点缺失则价格状态为 `unknown`。
- 每只 ETF 的周度份额变化率：
  `current_fd_share / previous_week_fd_share - 1`。两端分别取对应周最后一个
  有效交易日，禁止使用自然周末日期做前向填充。
- 四只 ETF 必须完整；任何一只缺失时 ETF 聚合状态为 `unknown`，缺失不能
  被当作净赎回。
- 聚合规则（死区为 ±0.5%）：
  - 至少 3/4 只周度份额变化率 > 0.5% → `净申购`
  - 至少 3/4 只周度份额变化率 < -0.5% → `净赎回`
  - 四只均在 ±0.5% 内 → `份额平稳`
  - 其余完整组合 → `分化`
- 信号：
  - 连续两周聚合为净申购 → `持续进场`
  - 本周聚合为净赎回 → `资金撤退`
  - 份额平稳但价格上涨 → `存量博弈`

#### 两融余额周度变化

- 主源使用 Tushare `margin`（需 `TUSHARE_TOKEN`），按周汇总。
- 若 token 无权限，允许使用 AkShare
  `macro_china_market_margin_sh` + `macro_china_market_margin_sz`
  的沪深历史汇总作为可审计 fallback；必须保存实际 `source`。
- 四档判断：

| 状态 | 条件 | 含义 |
|------|------|------|
| `健康上升` | 周度增速 1–5% | 杠杆温和扩张 |
| `过热预警` | 周度增速 > 5% | 杠杆扩张过快 |
| `盘整` | 周度变化在 -3% 至 1% 之间 | 杠杆未形成方向 |
| `恐慌出清` | 周度降幅 > 3% | 可能是底部信号 |

#### 超大单周度净流入

- 使用 AkShare `stock_market_fund_flow` 汇总一周的超大单净流入。
- 当前部署网络的历史端点可能只返回最新一行，因此周报优先聚合本地
  `flow_snapshots` 中五个完整交易日的原始事实。少于五个交易日时为
  `unknown`，不得把缺失日补零。
- 指数涨跌使用沪深300（`sh000300`）最近 5 个完整交易日收盘价，主源为
  AkShare `stock_zh_index_daily`（Sina）。绝对周涨跌不超过 0.5% 定义为
  “指数平”。
- 对比当周指数涨跌：
  - 指数平但超大单净流入 → `聪明钱进场，价格滞后`
  - 指数涨但超大单净流出 → `散户拉盘，主力减仓`
  - 两者同向上涨 → `共振进攻`

#### 板块强弱周度排序

- 使用 AkShare `stock_sector_fund_flow_rank` 获取行业和概念板块的周度资金流。
- 使用 `indicator="5日"` 取得本周排序；连续性标签必须读取上一期
  `weekly_macro_flow` 原始排名快照，不能用本周名次反推上周。
- 当前实测 `indicator="5日"` 返回 `名称`、`5日涨跌幅`、
  `5日主力净流入-净额`、`5日超大单净流入-净额` 等列；Schema 变化必须
  明确失败或降级 `unknown`，不能把缺列当作 0。
- 连续性判断（对比上周排名）：
  - 连续两周前5 → `持续主线`
  - 本周进入前5、上周不在 → `新兴热点`
  - 上周前5、本周跌出 → `热点退潮`
  - 资金流入但价格跑输大盘 → `资金滞留，吸筹`

### 报告格式

```text
# 宏观资金周报

周期：YYYY-MM-DD 至 YYYY-MM-DD

## 一句话结论

## ETF 资金动向

## 两融余额状态

## 超大单周度信号

## 板块强弱排序

## 数据质量
```

### 评分与结论

综合以上四项输出一个周度市场状态：

- `积极进场`：ETF 净申购 + 超大单净流入 + 两融健康 + 板块主线明确
- `谨慎观望`：信号混杂，至少一项出现矛盾
- `风险上升`：两融过热或 ETF 持续净赎回
- `恐慌区间`：两融快速下降 + ETF 净赎回 + 超大单净流出

---

## 月报：`monthly_macro_flow`

### 报告标题

`宏观流动性月报`

### 回答的问题

- 现在是不是牛市进度条在往前走？
- 存款搬家是否在加速？
- 杠杆水位是否健康？

### 数据源

#### 居民存款增速

- 数据来源：中国人民银行《金融机构人民币信贷收支表》中的
  “住户存款”余额。配置目标年和上一年官方 HTML/XLS 地址，严格按行名、
  月份和“亿元”单位解析。
- 不使用 `macro_china_money_supply` 推导居民存款；AkShare
  `macro_rmb_deposit` 也不作为本策略来源。
- 枯荣线：同比增速 **12%**。
  - 低于 12% → 存款增速放缓，资金有搬家迹象
  - 大于或等于 12% → 居民资产仍以存款为主，牛市还早

#### 非银存款变化

- 非银存款上升 = 资金进入金融市场（券商、基金等）。
- 赚钱效应扩散的信号之一。
- 数据只使用中国人民银行《金融机构人民币信贷收支表》中的
  “非银行业金融机构存款”余额，计算本月相对上月的变化额和环比。
- M1-M2 与非银存款是两个并列维度，不能互相代理。非银直接数据缺失时该维度
  为 `unknown`。

#### 融资余额占流通市值

- 融资余额：沪深两市同一交易日的“融资余额”汇总，不包含融券余额。
- 流通市值：同一交易日上交所“主板A + 科创板”与深交所
  “主板A股 + 创业板A股”的官方流通市值汇总。
- 不使用总市值、成交额或固定常量估算；Tushare `daily_basic.circ_mv`
  不作为首版依赖。
- 两条警戒线：
  - 比例 > **4%** → 危险区
  - 月度增速 > **20%** → 杠杆过热

#### M1-M2 剪刀差（辅助）

- M1 增速 - M2 增速。
- 持续扩大 → 经济活跃度上升，有利于股市。
- 持续收窄或负值 → 活钱减少，市场可能低迷。

### 报告格式

```text
# 宏观流动性月报

月份：YYYY-MM

## 一句话结论

## 牛市进度条

## 居民存款趋势

## 非银存款与活钱指标

## 杠杆水位

## 数据质量
```

### 月度市场状态分级

- `过热警报`：杠杆超 4% 或增速超 20%
- `牛市加速`：杠杆健康，且居民存款同比低于 12%、非银存款环比上升、
  M1-M2 剪刀差改善三项全部成立。
- `慢牛蓄力`：杠杆健康，且三项宏观信号中两项成立。
- `震荡磨底`：杠杆健康，且三项宏观信号中零项或一项成立。
- 非过热结论要求四个维度全部有效。任何必要维度缺失时只生成数据观察报告，
  不把缺失项计零，也不输出趋势状态。

完整数据契约、日期对齐、状态优先级和验收标准见
`docs/superpowers/specs/2026-07-26-monthly-macro-flow-design.md`。

---

## 数据源可用性评估

| 指标 | 数据源 | 依赖 | 可用性 |
|------|--------|------|--------|
| ETF 份额 | Tushare `fund_share`（`fd_share`，万份） | `TUSHARE_TOKEN` + 至少 2000 积分 | ❌ 当前 token 无权限；严格降级 unknown |
| ETF 成交额确认 | AkShare `fund_etf_hist_em`，空表时新浪 ETF 历史 fallback | 无 | ✅ 已有适配器 |
| 板块周度资金流 | AkShare `stock_sector_fund_flow_rank` | 无 | ✅ 无需配置 |
| 超大单周度流入 | AkShare `stock_market_fund_flow` | 无 | ✅ 无需配置 |
| 两融余额 | Tushare `margin`；AkShare 金十沪深历史 fallback | `TUSHARE_TOKEN`（主源） | ⚠️ 当前 token 无 margin 权限，fallback 已验证 |
| M2/M1 增速 | AkShare `macro_china_money_supply` | 无 | ✅ 2026-07-26 已验证 |
| 居民存款 | 央行《金融机构人民币信贷收支表》“住户存款” | 配置当前年与上一年官方 URL | ✅ 官方 HTML 已验证可解析 |
| 非银存款 | 同一央行表“非银行业金融机构存款” | 配置当前年与上一年官方 URL | ✅ 官方 HTML 已验证可解析 |
| 融资余额 | AkShare 沪深融资历史 | 无 | ✅ 2026-07-26 已验证 |
| A 股流通市值 | 沪深交易所指定日市场概况 | 无 | ✅ 2026-07-26 已验证 |

2026-07-26 部署预检：当前 token 调用 `fund_share` 返回“无接口访问权限”。
Phase 1 必须完整实现该数据源和权限失败降级，但真实上线时 ETF 份额项保持
`unknown`，直至 token 权限补齐并重新验收；不得启用成交量代理。

---

## Rollout

### Phase 1（周报）
- 新增 `src/lurker/ingest/macro_flows.py`：使用 Tushare `fund_share`
  抓取真实 ETF 份额，抓取板块周度资金流、超大单原始事实和两融周度数据。
- 新增 `src/lurker/application/weekly_macro_flow.py`：周度评分与报告生成。
- 在 `configs/strategies.yaml` 中启用 `weekly_macro_flow`，cadence=weekly。
- 新增 `tests/test_weekly_macro_flow.py`。
- 新增独立 `weekly-macro-flow` CLI；与旧 `weekly-report`（资金快照周报）
  并存，不替换、不复用其“延续/新主线/退潮”标签。
- 原始事实保存至独立 `macro_flow_snapshots/YYYY-MM-DD.json`；同日期重复
  运行覆盖而非追加。周末手动运行向前解析到最近中国交易日，不直接跳过。
- 推送使用日报接收人配置；提供 `--no-push` 演练模式。

### Phase 2（月报）
- 新增 `src/lurker/ingest/pboc_deposits.py`：下载并严格解析央行居民与非银
  存款余额。
- 新增 `src/lurker/ingest/macro_monthly.py`：拉取 M1/M2、沪深融资余额和交易所
  A 股流通市值，并完成月份与交易日对齐。
- 新增 `src/lurker/application/monthly_macro_flow.py`：月度打分与报告。
- 在 `configs/strategies.yaml` 中启用 `monthly_macro_flow`，cadence=monthly。
- 增加独立 `monthly-macro-flow` CLI、规范化快照、数据观察模式和 `--no-push`。

### Phase 3（整合）
- 在日报"市场资金温度"章节引用上一期周报和月报的宏观结论，形成三层嵌套视角。

---

## Non-Goals

- 不做个股级别的周度/月度分析（个股用日报即可）。
- 不预测价格，只描述资金流向和市场状态。
- 不要求实时数据，允许 T+1 延迟。
