...
日报不得直接展示内部英文状态码。数据质量区只输出截止日期和以下中文短标签，
不追加解释段落：
...
未识别的内部状态统一展示为“状态异常”，避免技术状态码泄漏到对客报告。该映射
只影响报告文字，不改变信号有效性、市场温度、降级状态或推送门禁。
...
## 9. 真实回放验收记录（2026-07-29）

- 回放区间：2026-04-30 至 2026-07-28，共 60 个严格递增且不重复的
  中国交易日；
- 大盘历史资金：60/60，来源 `eastmoney_market_flow_history`，缺失 0；
- 核心 ETF：四个角色逐日完整，来源 `akshare_fund_etf_hist_sina`，
  失败日 0；
- 两融：60/60，来源 `akshare_jin10_margin_sh_sz`，方向变化缺失 0；
- 状态分布：进攻 11、观察 17、防守 32，最高占比 53.3%，低于 80%；
- 因缺失数据降级为观察的天数：0；
- replay SHA256：
  `sha256:3e56302458b044df5ed0ac63e38091cf423f66373221aa64d2cd741730b127e5`；
- 审批身份：`codex-goal-2026-07-29`。审批由命令在完整校验通过后原子写入，
  日报仍会独立重算并复核。
