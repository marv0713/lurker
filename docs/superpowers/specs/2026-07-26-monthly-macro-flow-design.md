...
- 自动任务使用 `--month-end-only`：程序通过中国交易日历查询报告月份的最后
  一个交易日；若上海当地今天不是该日，则在任何外部采集、落盘或通知前跳过；
- VPS cron 仅负责在每月 25—31 日的 17:50（Asia/Shanghai）唤醒候选任务，
  是否真正执行完全由上述交易日门禁决定；不能用自然月最后一天代替交易日判断；
- 人工补跑或月中验收不传 `--month-end-only`，因此不受自动任务门禁限制；
- 默认推送到日报接收人；
