...
class CoreEtfItem:
    ...
    error: str | None              # 单标的错误信息

@dataclass
class CoreEtfBatch:
    configured_symbols: list[str]   # 配置中预期的全部 ETF symbol（如 ["510300.SH", "510500.SH", ...]）
    items: list[CoreEtfItem]       # 成功采集的 ETF（可空）
    failures: list[dict[str, str]] # {"symbol": "510300.SH", "reason": "..."}
...
```python
# 在 margin dict 中增加字段（normalize_margin_frame 返回）
{
    # 现有字段保持
    "trade_date": "20260723",
    "financing_balance": ...,
    ...
    # 新增
    "margin_signal": "supportive"  # "supportive" | "weakening" | "overheated" | "unknown"
}
```
...
- 调用方（`run_professional_flow_daily()`）在调温度前将 stale 数据的方向/状态降级为 `"unknown"` 或 `"neutral"`
...
| `src/lurker/application/market_temperature.py` | `classify_market_temperature()`、`classify_etf_status()`、`classify_margin_signal()`、`_flow_direction()` |
...
| 文件 | 变更 |
|------|------|
| `src/lurker/application/professional_flow_daily.py` | 删除旧的 `classify_market_temperature()`；`run_professional_flow_daily()` 调用新模块 |
| `src/lurker/application/flow_snapshot.py` | `collect_flow_snapshot()` 接入 `fetch_core_etfs()`；`_capture()` 对 `core_etfs` 源返回空 `CoreEtfBatch` |
| `src/lurker/ingest/flows.py` | `normalize_margin_frame()` / `fetch_margin()` 增加 `margin_signal` 字段（通过调用 `application/market_temperature.py` 的 `classify_margin_signal()`） |
| `src/lurker/cli.py` | `daily-job` 增加 `--no-push`；`refresh-flows` 接入 ETF 采集；新增 `build-temperature-replay` 命令；增加温度闸门检查 |
| `src/lurker/application/weekly_flow_report.py` | `_status_counts()` 适配新 `classify_market_temperature()` 签名 |
...
| `tests/test_flows.py` | 新增 `margin_signal` 字段测试 |
...
**测试命令**：
```bash
cd /Users/marv/Documents/lurker && PYTHONPATH=src .venv/bin/pytest tests/test_market_temperature.py -v
# 预期：全部 RED（ModuleNotFoundError 或 ImportError）
```
...
# 2. 测试带后缀
# df2 = ak.fund_etf_hist_em(symbol="510300.SH", ...)  # 是否报错？

# 3. 确认成交额列名和单位
# 预期列名：日期, 开盘, 收盘, 最高, 最低, 成交量, 成交额, ...
# 确认成交额单位（元 vs 万元）
...
**GREEN 步骤**：
1. Task 2 预检完成，冻结 AkShare 适配规则
2. 创建 `configs/core_etfs.yaml`
3. 在 `src/lurker/config.py` 增加 `load_core_etfs(path) -> list[dict[str, str]]`
4. 实现 `src/lurker/ingest/etf_flows.py`
5. 所有 ETF 相关测试变绿
...
```python
# 旧代码删除
# def classify_market_temperature(...):  # 删除
...
if isinstance(core_etfs_data, dict):
    # schema v2：使用 from_dict 恢复嵌套 dataclass
    etf_batch = CoreEtfBatch.from_dict(core_etfs_data)
    etf_status = classify_etf_status(etf_batch)
elif isinstance(core_etfs_data, list):
    # schema v1 兼容：旧列表格式 → unknown
    etf_status = "unknown"
else:
    etf_status = "unknown"

margin_data = flow_snapshot.get("margin", {})
margin_signal = margin_data.get("margin_signal") or classify_margin_signal(margin_data)
...
**`flows.py` 变更**：
- `normalize_margin_frame()` 返回 dict 中增加 `"margin_signal"` 字段
- `margin_signal` 值由调用方（`collect_flow_snapshot` 或 `daily_job`）通过 `classify_margin_signal()` 计算后注入
- `ingest/flows.py` 不 import `application/market_temperature.py`（避免循环依赖）；信号计算在 `flow_snapshot.py` 中完成

**`flow_snapshot.py` 变更**：
```python
# collect_flow_snapshot() 中
...
- `_status_counts()` 中加载快照的 `margin` 后调用 `classify_margin_signal()` 提取信号
- `core_etfs` 反序列化为 `CoreEtfBatch` 后调用 `classify_etf_status()`
...
def _default_etf_fetcher() -> CoreEtfBatch:
    ...
    # 注意：fetch_core_etfs() 内部只降级 EtfProviderError/EtfSchemaError/网络异常
    # TypeError/KeyError/ValueError 等程序/配置错误向上传播，不被捕获
```
...
| `test_collect_flow_snapshot_returns_unknown_on_etf_provider_error` | `EtfProviderError` → snapshot 中 `core_etfs` 为 unknown batch |
...
**存量测试适配策略**：
- 所有使用 `core_etfs: []` 的 fixture：改为使用 `monkeypatch` 注入 `CoreEtfBatch(items=[], failures=[], ...)` 或直接 mock 温度函数
...
- 按 `trade_date` 对齐三个数据源：
  ...
  3. **两融历史**：Tushare `pro.margin(trade_date=...)` 按交易日逐日查询
...
{
  "date": "2026-04-24",
  "market_flow": {"main_net_inflow": ..., "super_large_net_inflow": ...},
  "core_etfs": {
    "items": [
      {"symbol": "510300.SH", "trade_date": "2026-04-24", "current_turnover": ..., "avg_turnover_20d": ..., "turnover_expansion": ...}
    ],
    "failures": []
  },
  "margin": {"financing_balance": ..., "margin_balance": ..., "margin_balance_change": ...}
}
```
...
```json
{
  ...
  "rules_fingerprint": "sha256:abc123...",
...
- `rules_fingerprint` = 温度分类规则代码的 SHA256（规则逻辑变更后指纹变化 → artifact 失效）
...
def _check_temperature_gate(
    ...
    - 任一状态占比 > max_ratio → False, "状态X占比Y%超过80%"
...
| 字段 | v1 格式 | v2 格式 | 兼容策略 |
|------|---------|---------|---------|
| `core_etfs` | `[]`（空列表） | `{"items": [...], "failures": [...], ...}` | `isinstance(dict)` → v2；`isinstance(list)` → unknown |
| `margin.margin_signal` | 不存在 | `"supportive"` 等 | `margin.get("margin_signal")` 返回 None → `classify_margin_signal()` 内部处理 |

### 7.2 存量测试兼容

- mock `collect_flow_snapshot` 的测试：需要将 `core_etfs: []` 改为 `core_etfs: {"items": [], "failures": [], ...}`
- mock `classify_market_temperature` 的测试：需要调整参数为三态字符串

### 7.3 周报兼容

- `weekly_flow_report.py` 加载快照后反序列化 `core_etfs`，调用 `classify_etf_status()`
- 快照 `margin` 已有 `margin_signal`（v2）→ 直接使用；没有（v1）→ 调用 `classify_margin_signal()` 计算
